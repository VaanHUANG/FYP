...
url_floorplan0  = 'network-snapshot-018565.pkl' # karras2019stylegan-ffhq-1024x1024.pkl
...
def main():
    ...
    #Gs = load_Gs_local(url_floorplan0)
    ...
    #draw_uncurated_result_figure(os.path.join(config.result_dir, 'figure02-uncurated-ffhq.png'), load_Gs_local(url_floorplan0), cx=0, cy=0, cw=256, ch=256, rows=3, lods=[0,1,2,2,3,3], seed=5)
    #draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing.png'), load_Gs_local(url_floorplan0), w=256, h=256, src_seeds=[69,71,67,65,28], dst_seeds=[8,89,18,17,14,10], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,18)])
    #draw_noise_detail_figure(os.path.join(config.result_dir, 'figure04-noise-detail.png'), load_Gs_local(url_floorplan0), w=256, h=256, num_samples=100, seeds=[1157,1012])
    #draw_noise_components_figure(os.path.join(config.result_dir, 'figure05-noise-components.png'), load_Gs_local(url_floorplan0), w=256, h=256, seeds=[1967,1555], noise_ranges=[range(0, 18), range(0, 0), range(8, 18), range(0, 8)], flips=[1])
    draw_truncation_trick_figure(os.path.join(config.result_dir, 'figure08-truncation-trick.png'), load_Gs_local(url_floorplan0), w=256, h=256, seeds=[798, 1913, 388], psis=[1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0, -0.5, -1])
